...
int main(int argc, char* argv[])
{
  ...
  Grid* grid = Grid::create(comm, vm["grid"].as<string>(),
                            vm["dim0"].as<string>(), vm["dim1"].as<string>(),
                            vm["mask"].as<string>(), vm["blk0"].as<int>(),
  ...
  partitioner->get_top_neighbors(top_ids, top_halos);
  partitioner->get_bottom_neighbors(bottom_ids, bottom_halos);
  partitioner->get_left_neighbors(left_ids, left_halos);
  partitioner->get_right_neighbors(right_ids, right_halos);

  // MPI ranks of neighbors in order: top, bottom, left, right
  ...
  // Create distributed neighborhood communicator
  MPI_Comm comm_dist_graph;
  int err = MPI_Dist_graph_create_adjacent(
      comm, ids.size(), ids.data(), MPI_UNWEIGHTED, ids.size(), ids.data(),
  ...
  for (int i = 0; i < top_ids.size(); i++) {
    ...
    MPI_Type_create_subarray(ndims, sizes, subsizes, send_top_start,
                             MPI_ORDER_C, MPI_DOUBLE, &subar_top[i]);
    ...
    MPI_Type_create_subarray(ndims, sizes, subsizes, recv_top_start,
                             MPI_ORDER_C, MPI_DOUBLE, &ghost_top[i]);
  ...
  for (int i = 0; i < bottom_ids.size(); i++) {
    ...
    MPI_Type_create_subarray(ndims, sizes, subsizes, send_bottom_start,
                             MPI_ORDER_C, MPI_DOUBLE, &subar_bottom[i]);
    ...
    MPI_Type_create_subarray(ndims, sizes, subsizes, recv_bottom_start,
                             MPI_ORDER_C, MPI_DOUBLE, &ghost_bottom[i]);
  ...
  for (int i = 0; i < left_ids.size(); i++) {
    ...
    MPI_Type_create_subarray(ndims, sizes, subsizes, send_left_start,
                             MPI_ORDER_C, MPI_DOUBLE, &subar_left[i]);
    ...
    MPI_Type_create_subarray(ndims, sizes, subsizes, recv_left_start,
                             MPI_ORDER_C, MPI_DOUBLE, &ghost_left[i]);
  ...
  for (int i = 0; i < right_ids.size(); i++) {
    ...
    MPI_Type_create_subarray(ndims, sizes, subsizes, send_right_start,
                             MPI_ORDER_C, MPI_DOUBLE, &subar_right[i]);
    ...
    MPI_Type_create_subarray(ndims, sizes, subsizes, recv_right_start,
                             MPI_ORDER_C, MPI_DOUBLE, &ghost_right[i]);
  ...
  err = MPI_Neighbor_alltoallw(data.data(), scounts.data(), sdispls.data(),
                               sendtypes.data(), data.data(), rcounts.data(),
